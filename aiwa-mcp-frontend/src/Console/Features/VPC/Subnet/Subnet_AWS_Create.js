...
function Subnet_Create() {
    ...
    const handleCreateSubnet = async () => {
        if (!subnetName || !cidrBlock || selectedVPC === "non-selected" || selectedAZ === "non-selected") {
            alert("VPC, Availability Zone, Subnet 이름과 CIDR 블록을 모두 입력해주세요.");
            return;
        }
        ...
        <div>
            ...
            <Flex direction="row" height="100vh">
                ...
                <Flex
                    ...
                    gap="20px"
                >
                    <Heading level={2}>AWS Subnet 생성</Heading>

                    <TextField
                        fontWeight="bold"
                        label="Subnet 이름"
                        placeholder="Subnet 이름을 입력하세요"
                        value={subnetName}
                    ...
                    <Button
                        ...
                        isDisabled={!subnetName || !cidrBlock || selectedVPC === "non-selected" || selectedAZ === "non-selected"} // Disable if no VPC or AZ selected
                    >
                        Create Subnet
                    </Button>
                </Flex>
